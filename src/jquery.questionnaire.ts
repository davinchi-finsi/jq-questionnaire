    ...
    $.widget(
        "ui.jqQuestionnaire", {
            ...
            options: {
                ...
                pointsForSuccess: 1,//amount of points to increase when success a question
                pointsForFail: 0,//ammoint of points to rest when fail a question
                cutOffMark: 50,//minimum % of points to success
                immediateFeedback: false,
                disableOptionAfterSelect: true,//only applies if immediateFeedback == true
                autoGoNext: true,//only applies if immediateFeedback == false
                showCorrection: true,//show the question corrections at the end
                showResult: true,//show the result (calification)
                multichoice: false,
            ...
            _renderVar: function (query, data, store, context) {
                ...
                for (let element of $toRender) {
                    let $element = $(element),
                        optionName = ($element.data(data) || "").trim(),//get the data name and trim it
                        optionAsTrue,
                        optionAsFalse;
