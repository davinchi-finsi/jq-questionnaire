...
$.widget("ui.jqQuiz", {
    ...
    calificate: function (runtime) {
        let currentScore = 0, maxScore = this._maxScore, runtimeToCalificate = runtime || this._runtime, questions = this._questions, calification, nSuccess = 0, nFails = 0;
    ...
    _calificateSingleChoice: function (runtime) {
        let currentScore = 0, runtimeToCalificate = runtime || this._runtime, questions = this._questions, nSuccess = 0, nFails = 0;
        //for each question
        for (let questionIndex = 0, questionsLength = questions.length; questionIndex < questionsLength; questionIndex++) {
            let currentQuestion = questions[questionIndex], questionRuntime = runtimeToCalificate[currentQuestion.id], //get runtime for question
    ...
    _calificateSingleChoiceQuestion: function (questionId, runtime) {
        let runtimeToCalificate = runtime || this._runtime, question = this.getQuestionById(questionId), result;
        if (question) {
    ...
    _updateRuntime: function (questionId, optionId, optionValue, selected) {
        let questionRuntime = this._runtime[questionId] || {}, options = questionRuntime.options || [], optionsValues = questionRuntime.optionsValues || [];
    ...
    _onOptionChange: function (e) {
        ...
        if (instance.options.disabled != true && instance._state == instance.STATES.running) {
            ...
            questionId = $question.attr("id"), optionId = $option.attr("id"), optionValue = $option.find("input").attr("value"), pendingQuestionIndex;
        ...
        for (let questionId in states) {
            ...
            if (question) {
                ...
                for (let optionStateIndex = 0, optionsLength = state.options.length; optionStateIndex < optionsLength; optionStateIndex++) {
                    let optionId = state.options[optionStateIndex], optionIndex = question.optionsMap[optionId];
                    if (optionIndex != null) {
                        //mark as checked
